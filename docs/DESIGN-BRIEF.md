...
Three reference images are supplied alongside this brief in `design/`. They pull in different
directions, deliberately — here is exactly what to take from each.

### Reference A — Mercedes AMG telemetry wall (`original-2a5e…webp`)

**Take:** the genre. Panel-partitioned dark layout, every panel labelled with a small caps header
and an icon, numeric values large and monospaced with units set small and dim beside them, live
traces with explicit axis ranges, a horizontal RPM bar with a red-line zone. This is what
broadcast-grade instrumentation looks like and it is the right family for our dashboard.

**Leave:** the single-driver focus and the team branding. Our product is a full 20-car field, and
it carries no team identity of its own.

### Reference B — Race strategy dashboard, Monaco (`10da1e97…webp`) ★ closest to the target

**Take:** most of it. Specifically —
- The header band: live dot, session name and circuit, lap `73/78` as the largest number on screen,
  track and air temperature, and a flag-state chip on the right.
- The **timing tower treatment**: `POS · NUM · DRIVER · GAP · INTV · LAST LAP · BEST LAP · TIRE ·
  AGE`, driver TLA large with the team name small and dim beneath, car number in team colour,
  purple for an overall-fastest lap, tyre as a lettered circular badge with age beside it.
- Monospaced numerics throughout, uppercase small-caps panel headers, hairline panel borders on a
  near-black ground.
- The restrained use of red: reserved for genuine alerts, not spent on chrome.

**Leave:** the red ambient glow behind the panels, and the strategy/AI panels — those are a
different product. Our equivalent panels are race control, weather and telemetry.

### Reference C — F1 portal concept (`5c71e5df…webp`) ⚠ read the caveat

**Take:** only the landing page's sense of occasion — a large next-race card with the circuit
outline drawn as a thin white line, a compact standings list, and generous type for a hero element.
That is a legitimate direction for `/` (§9.7).

**Do not take, and do not reproduce:**
- **The F1 logo and wordmark.** We may not use F1 branding anywhere — see §11. This is a legal
  constraint, not a stylistic preference.
- Driver photography as a hero. We have no rights to it, and the feed supplies only headshot URLs.
- The video player, the highlights list and the music player — all explicitly out of scope (§11).
- The search field, avatar and notification badge — there are no user accounts in this product.
- Heavy glassmorphism and blur on data surfaces. It costs legibility on a screen read in one-second
  glances, and it costs GPU on a page already animating 20 cars at 60fps. Reserve any such
  treatment for the landing page, never for the dashboard.

### The synthesis

**Reference B is the dashboard. Reference A is the panel and instrumentation vocabulary. Reference
C is a hint at the landing page's ambition and nothing else.** Where B and C conflict, B wins for
every screen except `/`.
