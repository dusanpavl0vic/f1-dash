...
<h3>What the indexes are actually asked</h3>
<p>An index that is only ever written to is pure cost. Four endpoints exist for the questions the
file layout cannot answer, and each returns an empty result rather than an error when its store is
down — showing nothing is the correct answer for a deployment that runs no databases.</p>

<table>
<thead><tr><th style="width:34%">Endpoint</th><th>Question</th></tr></thead>
<tbody>
<tr><td><code>/api/insights/circuit/{slug}</code></td><td>Fastest laps at one circuit, every indexed season</td></tr>
<tr><td><code>/api/insights/driver/{code}</code></td><td>One driver's pace season by season — <u>median, not mean</u>, because one safety-car lap moves a mean by seconds and a median not at all</td></tr>
<tr><td><code>/api/insights/strategies/{year}</code></td><td>Who ran which strategy across a whole season</td></tr>
<tr><td><code>/api/insights/speed</code></td><td>Every lap above a speed threshold — the one genuine time-series question</td></tr>
</tbody></table>

<div class="note blue"><span class="lbl">One documented path was wrong and is recorded, not deleted</span>
An earlier draft said the session catalogue would be read from Postgres with a filesystem fallback.
Postgres only knows sessions that have been <em>indexed</em>, while the archive index lists every
session that exists — so making it primary would <b>hide</b> sessions from the picker. The database
would have been authoritative about something it has an incomplete view of, which is the exact
failure the "archive is authoritative" rule exists to prevent.</div>
...
<h3>What the telemetry view can and cannot show</h3>
<p>Tyre temperatures, brake temperatures, pressures and fuel load are <u>team-internal and never
published</u>. They are absent from the interface rather than invented. What replaces them is the
channel data F1 does publish — speed, gear, throttle, brake and RPM — alongside estimates that
declare themselves on the panel rather than in a footnote.</p>
<p>Tyre wear is inferred from age against a nominal life the panel states out loud. Degradation is a
least-squares slope over recent laps with outliers removed <em>before</em> fitting, because a single
safety-car lap otherwise turns a flat trend into a claimed four seconds a lap.</p>

<div class="note"><span class="lbl">A value question that was really an era question</span>
DRS was displayed from channel 45's value. In a DRS season <code>0</code> means closed; from 2026
the channel is absent and every reading is <code>0</code> — which rendered as a permanently closed
wing on a car that never had one. It is now gated on era detection. Verified both ways: 2024 Monza
publishes channels 0, 2, 3, 4, 5 and 45; 2026 Monza publishes 0, 2, 3, 4, 5.</div>
...
110 backend tests and 66 in the browser suite, with a typechecked and linted frontend. All three
